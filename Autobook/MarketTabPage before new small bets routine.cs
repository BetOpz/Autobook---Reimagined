				...
				switch (param.type)
				{
					...
					case BetType.Back:
						...
						foreach (BetfairE.RunnerPrices rp in param.runnerPrices)
						{
							...
							size = HandleSmallBet (rp.selectionId, betAmount, BetType.Back);
							if (size == 0.0d)
							{
					...
					case BetType.Lay:
						...
						foreach (BetfairE.RunnerPrices rp in param.runnerPrices)
						{
							...
							if (IsSmallLay)
							{
								size = HandleSmallBet (rp.selectionId, betAmount, BetType.Lay);
								if (size == 0.0d)
								{
									_logger.Info (string.Format ("PlaceBet: Lay wait for overbet used up  {0} {1} {2}",
										GetRunnerName (rp.selectionId), rp.bestPricesToLay[0].price.ToString (DoubleFormat),
										rp.bestPricesToLay[0].amountAvailable.ToString (DoubleFormat)));
									continue;
								}
							}
							else
...
//      }
		...
		*/
		...
		private double HandleSmallBet (int selectionId, double amount, BetType betType)
		{
			double totalAmount = amount;
			double betAmount;
			Dictionary<int, double> smallAmounts;

			switch (betType)
			{
				case BetType.Back:
					smallAmounts = _smallBackAmounts;
					break;

				case BetType.Lay:
					smallAmounts = _smallLayAmounts;
					break;

				default:
					_logger.Error ("HandleSmallBet: unknown bet type " + betType.ToString ());
					return amount;
			}
			/*
						foreach (KeyValuePair<int, double> kvp in _smallAmounts)
						{
							DisplayDebugMessage (string.Format ("HandleSmallBet: _smallAmounts1 {0} {1}", kvp.Key, kvp.Value.ToString (DoubleFormat)));
						}
			*/
			if (smallAmounts.ContainsKey (selectionId))
			{
				totalAmount = amount - smallAmounts[selectionId];
				if (totalAmount >= Globals.MinBetAmount)
				{
					smallAmounts.Remove (selectionId);
					betAmount = totalAmount;

					_logger.Info (string.Format ("HandleSmallBet: {3} used up amount {0} betAmount {1} selection {2}",
						amount.ToString (DoubleFormat), betAmount.ToString (DoubleFormat),
						GetRunnerName (selectionId), betType.ToString ()));
				}
				else
				{
					if (totalAmount >= 0)
					{
						smallAmounts[selectionId] = Globals.MinBetAmount - totalAmount;
						betAmount = Globals.MinBetAmount;
					}
					else
					{
						smallAmounts[selectionId] -= amount;
						betAmount = 0.0d;
					}

					_logger.Info (string.Format ("HandleSmallBet: {4} smallAmounts {0} amount {1} betAmount {2} selection {3}",
						smallAmounts[selectionId].ToString (DoubleFormat), amount.ToString (DoubleFormat),
						betAmount.ToString (DoubleFormat), GetRunnerName (selectionId), betType.ToString ()));
				}
			}
			else
			{
				if (totalAmount < Globals.MinBetAmount)
				{
					...
					smallAmounts.Add (selectionId, overBet);
					...
					_logger.Info (string.Format ("HandleSmallBet: {3} overbet {4} amount {0} betAmount {1} selection {2}",
						amount.ToString (DoubleFormat), betAmount.ToString (DoubleFormat),
						GetRunnerName (selectionId), betType.ToString (), overBet.ToString (DoubleFormat)));
				}
				else
				{
					betAmount	 = totalAmount;
			...
			/*
						foreach (KeyValuePair<int, double> kvp in _smallAmounts)
						{
							DisplayDebugMessage (string.Format ("HandleSmallBet: _smallAmounts2 {0} {1}", kvp.Key, kvp.Value.ToString (DoubleFormat)));
						}

						if (betAmount != amount)
							DisplayDebugMessage (string.Format ("HandleSmallBet: amount {0} betAmount {1} selection {2}",
